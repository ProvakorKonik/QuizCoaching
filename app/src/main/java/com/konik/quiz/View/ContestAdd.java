...
public class ContestAdd extends AppCompatActivity {
    ...
    private void UploadCropedImageFunction(Uri filePath) {
        if(filePath != null)
        {
            ...
            ref = storageReference.child("Rakib/"+ dsContestName+" "+dsTimeMiliSeconds +"."+getFileExtention(filePath));
                        ...
                        public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                            ref.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                                ...
                                public void onSuccess(Uri uri) {
                                    ...
                                    note.put("QuizName", dsContestName);
                                    note.put("QuizPhotoUrl", dPhotoURL);
                                    note.put("QuizSyllabus", dsContestSyllabus);
                                    note.put("QuizExtra", "0");
                                    note.put("QuizCreator", dUserUID);
                                    note.put("QuiziTotalQues", diContestTotalQuestion);
                                    note.put("QuiziPriority", diPriority);
                                    note.put("QuiziTotalParticipant", diContestTotalParticipent);
                                    note.put("QuiziDuration", diContestDuration);
                                    FieldValue ddDate =  FieldValue.serverTimestamp();
                                    note.put("QuiziDate", ddDate); // Date of Contest



                                    db.collection("Rakib").document("Info")
                                            .collection("AllContest").add(note)
                                            .addOnSuccessListener(new OnSuccessListener<DocumentReference>() {
                                                ...
                                                public void onSuccess(DocumentReference documentReference) {
                                                    ...
                                                    finish();
                                                    Intent intent = new Intent(ContestAdd.this, MainActivity.class);    //Error Intent not sent
                                                    intent.setFlags(intent.getFlags() | Intent.FLAG_ACTIVITY_NO_HISTORY);
                                                    startActivity(intent);
                                                }
